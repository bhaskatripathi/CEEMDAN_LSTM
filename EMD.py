...
def LSTM_Model(X_train, Y_train,i):
    ...
    model = Sequential()
    model . add ( LSTM ( 50 , activation = 'tanh' , input_shape = ( X_train . shape [ 1 ], X_train . shape [ 2 ])))   # 10 steps already determined
    '''
...
def isolutionforest(DO):
    ...
    clf  =  IsolationForest ( random_state = rng , contamination = 0.025 )   # contamination is the proportion of abnormal samples
    clf.fit(DO)
